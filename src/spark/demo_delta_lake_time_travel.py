...
# %% query v1
df = spark.read.format("delta").option("versionAsOf", "2").load(table_path)
df.show()
# query v2
df = spark.read.format("delta").option("versionAsOf", "1").load("delta/people")
df.show()
# query latest
...
# %% vacuum
q = spark.sql(f"VACUUM delta.`{table_path}` DRY RUN")
