...
df = spark \
    ...
    .option("sep", ",") \
    .schema(userSchema) \
    .csv("spark_application/csv")  # Equivalent to format("csv").load("/path/to/directory")
...
df = (spark
      .readStream
      .schema(schema)
      .csv("s3a://deutsche-boerse-xetra-pds/2022-02-10/", header=True))

#%%
query = (df
        .writeStream
        .queryName("stock_price")
        .format("delta")
        .trigger(once=True)
        .partitionBy("date")
        .outputMode("append")
...
#%%
dk = stock_price.select("isin", "date", "time", "startPrice", "endPrice").where("isin in ('DE000ETFL144', 'DE0006042708')").sort("time")
dk.show()
...
start_df = dk.withColumn("asc_rank", functions.rank().over(asc_window)).select("isin", "startPrice").where("asc_rank = 1")
end_df = dk.withColumn("desc_rank", functions.rank().over(desc_window)).select("isin", "endPrice").where("desc_rank = 1")
#%%
